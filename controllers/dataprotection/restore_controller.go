...
func (r *RestoreReconciler) handleBackupActionSet(reqCtx intctrlutil.RequestCtx,
	...
	// For in-flight actions, check the recorded Job before rebuilding from the
	// current target pod selector. The target pod may disappear after the Job is
	// created, but the existing Job can still complete the action.
